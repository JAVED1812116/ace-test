...
// const { REFRESH_TOKEN_SECRET, ACCESS_TOKEN_SECRET, ACCESS_TOKEN_EXPIRY_TIME, REFRESH_TOKEN_EXPIRY_TIME } = require('../config/index');
const { User_Tokens_Schema } = require('../models/user_tokens_model');
const { ACCESS_TOKEN_SECRET, ACCESS_TOKEN_EXPIRY_TIME } = require('../config');
...
class JWT_Generate_Token_Handle {
    static save_user_tokens = async (user_id) => {
        ...
        const access_token = JWT_Service.sign_jwt_token(token_payload, ACCESS_TOKEN_SECRET,ACCESS_TOKEN_EXPIRY_TIME)
        return access_token
    }
}
